...
from src.utils import DEBUG, get_date_from_expr, str_upper_case, map_of_type, convert_img_2_base64, decode_qrcode, \
    get_province_code, remove_accents, get_date_from_reg_date, padding_img
...
def get_data_mrz(img, bboxes, ocr):
    ...
    for idx, val in enumerate(new_bb):
        ...
        name, score = ocr.predict(get_image_box(img, val, is_gray=True, is_padding=True), return_prob=True)
        content += name + '\n'
        prob += score
...
class INFO_BOX_DET:
    ...
    def predict_front(self, img, threshold=0.85, debug=True, file=None):
        results = self.model.predict(source=img, save=False, verbose=False, imgsz=640)
        ...
        content += '</annotation>'
        with open(f'/home/thienbd90/debug/8_cropped/' + file[:-4] + '.xml',
                  'w') as f:
            f.write(content)
    ...
    def predict_back(self, img, threshold, file):
        ...
        content += '</annotation>'
        with open(f'/home/thienbd90/debug/9_cropped/' + file[:-4] + '.xml',
                  'w') as f:
            f.write(content)
        if DEBUG:
            img_copy = img.copy()
    ...
    def recognition_back(self, img, i_type, c_ocr, mrz_ocr):
        ...
        map_box = self.predict_back(img, threshold=0.7)
...
if __name__ == '__main__':
    text_box_det = INFO_BOX_DET(weight_path='./models/idcard_models/textbox/cccd_all_textbox2.pt')
    for root, dirs, files in os.walk('/home/thienbd90/debug/9_cropped'):
        for file in files:
            if file.endswith('.jpg'):
                img = cv2.imread(os.path.join(root, file))
                print(file)
                text_box_det.predict_back(img, threshold=0.5, file=file)
                print('=================================================================')
