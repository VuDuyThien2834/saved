...
from src.utils import DEBUG, get_date_from_expr, str_upper_case, map_of_type, convert_img_2_base64, decode_qrcode, \
    get_province_code, remove_accents, get_date_from_reg_date, cv2_img_2_pil, stringToImage, resize
...
def get_data_mrz(img, bboxes, ocr):
    ...
    for idx, val in enumerate(new_bb):
        ...
        name, score = ocr.predict(line_img, return_prob=True)
        print(name, score)
        if len(name) > 31 and score < 0.88:
            name = name[1:]
        content += name + '\n'
        prob += score
...
class INFO_BOX_DET_2024:
    ...
    def predict_back(self, img, threshold):
        ...
        for idx, bbox in enumerate(boxes):
            ...
            if len(bbox_mrz_line) < 3 and cls == MRZ_LINE:
                ...
                bbox_mrz_line = sorted(bbox_mrz_line, key=lambda k: k[0][1])
    ...
    def recognition_back(self, img, i_type, c_ocr, mrz_ocr):
        ...
        # MRZ
        # mrz_data = ocr_mrz(crop_img=img)  #g
